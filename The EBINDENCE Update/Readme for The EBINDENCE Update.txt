The EBIDENCE Update is out, and the server now supports the new Evidence feature that has been added to AO2.4.0 and above. You are highly recommended to use AO2.4.1 in order to take advantage of these new features.

Evidence Added:
AA
JFA
T&T
AJ
DD
SOJ
DGS
AAI
AAI2

============================
Bat deletion: The following folders will be deleted/renamed. Manually delete them if you do not trust the .bat file. RUN THE BAT FILE BEFORE UPDATING.

Roylott (changed to Grimesby Roylott)
[DR] Objection.mp3 > [DR] Announce the Truth (Variation).mp3
